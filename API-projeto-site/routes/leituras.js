...
router.get('/tempo-real/:idcultivo', function(req, res, next) {
	...
	if (env == 'dev') {
		// abaixo, escreva o select de dados para o Workbench
		instrucaoSql = `select temperatura_coletada, umidade_coletada, DATE_FORMAT(data_hora_coletada,'%H:%i:%s') as momento_grafico, fksensor from leitura where fksensor = ${idcultivo} order by id_DadosColetados desc limit 1`;
	} else if (env == 'production') {
		// abaixo, escreva o select de dados para o SQL Server
		instrucaoSql = `select top 1 temperatura, umidade, FORMAT(momento,'HH:mm:ss') as momento_grafico, fkcaminhao from leitura where fkcaminhao = ${idcultivo} order by id_DadosColetados desc`;
	} else {
		console.log("\n\n\n\nVERIFIQUE O VALOR DE LINHA 1 EM APP.JS!\n\n\n\n")
	}
