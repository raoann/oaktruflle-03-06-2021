...
router.get("/sendData", (request, response) => {
	...
	if (env == "dev") {
		...
		values (${temperature + 10}, ${Humidity},'${agora()}', 1),
		(${temperature - 10}, ${Humidity},'${agora()}', 2),
		(${temperature + 5}, ${Humidity},'${agora()}', 3),
		(${temperature - 5}, ${Humidity},'${agora()}', 4);`;
	} else {

		// Na variável abaixo, coloque o Insert que será executado no SQL Server
		// salvo exceções, é igual a Workbench

		instrucaoSql = `(temperatura_coletada, umidade_coletada, fkSensor)
		values (${temperature + 10}, ${Humidity + 20}, 1),
		(${temperature - 10}, ${Humidity + 20}, 2),
		(${temperature + 5}, ${Humidity - 20}, 3),
		(${temperature - 5}, ${Humidity - 20}, 4);`;
	}
